    ...
    async def _create_ai_card(
        ...
        *,
        ...
        if not conversation_id:
            return None
        if not is_group and not sender_staff_id:
            raise RuntimeError("create ai card failed: sender_staff_id missing for IM_ROBOT")
        ...
        async with self._http.post(
            ...
            if resp.status >= 400:
                raise RuntimeError(
                    f"create ai card instance failed status={resp.status} body={body_text[:500]}"
                )
            try:
                body_json = json.loads(body_text) if body_text else {}
            except json.JSONDecodeError:
            ...
            if isinstance(body_json, dict) and body_json.get("success") is False:
                raise RuntimeError(
                    f"create ai card instance failed body={body_text[:500]}"
                )
        ...
        if is_group:
            deliver_payload["openSpaceId"] = f"dtv1.card//IM_GROUP.{conversation_id}"
            deliver_payload["imGroupOpenDeliverModel"] = {
                "robotCode": self.robot_code,
            }
        else:
            deliver_payload["openSpaceId"] = f"dtv1.card//IM_ROBOT.{sender_staff_id}"
            deliver_payload["imRobotOpenDeliverModel"] = {
                "spaceType": "IM_ROBOT",
            }
        ...
        async with self._http.post(
            ...
            if resp.status >= 400:
                raise RuntimeError(
                    f"deliver ai card failed status={resp.status} body={body_text[:500]}"
                )
            try:
                body_json = json.loads(body_text) if body_text else {}
            except json.JSONDecodeError:
            ...
            result = body_json.get("result") if isinstance(body_json, dict) else None
            deliver_results = []
            if isinstance(result, list):
                deliver_results = result
            elif isinstance(result, dict) and isinstance(result.get("deliverResults"), list):
                deliver_results = result.get("deliverResults") or []
            if isinstance(body_json, dict) and body_json.get("success") is False:
                raise RuntimeError(
                    f"deliver ai card failed body={body_text[:500]}"
                )
